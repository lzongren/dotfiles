#!/usr/bin/env bash
# setup-remote.sh — provision an Amazon Linux 2 cloud desktop for remote
# Claude Code development: builds mosh + a modern tmux from source (the
# packaged versions are broken/ancient on AL2) and deploys the tmux config.
#
# Run FROM your laptop:  ./remote-dev/setup-remote.sh [ssh-host]
# Default host: zongrenl-dev
#
...
ssh "$HOST" 'true' || die "Cannot SSH to $HOST"

# ---------------------------------------------------------------------------
# Remote build. Heredoc runs on the dev desktop. Encodes the AL2 gotchas:
